...
router.post("/", async (req, res) => {
  try {
    ...
    const [departmentDetails] = await db
      .select()
      .from(departments)
      .where(eq(departments.id, createdDepartment.id));

    res.status(201).json({ data: departmentDetails });
  } catch (error) {
  ...
  try {
    ...
    if ((subjectCount?.count ?? 0) > 0) {
      return res.status(409).json({
        error: "Cannot delete department with associated subjects. Remove the subjects first.",
      });
    }
    ...
    res.status(200).json({ data: { id: deletedDepartment.id } });
  } catch (error) {
